...
const TWILIO_TOKEN_URL = 'https://sunset-newt-9635.twil.io/create-room-token';
...
const reducer = (state, action) => {
    switch (action.type) {
        case 'join':
            return { ...state, token: action.token, identity: action.identity, roomName: action.roomName };
...
const useTwilioVideo = () => {
    ...
    const connectToRoom = async () => {
        ...
        const room = await connect(
            ...
            console.error(`Unale to join the room: ${error.message}`)
        })
